...
This section describes how client systems should create and submit vital sign Observations
to the CIBMTR FHIR server in a manner that is consistent with US Core and CIBMTR reporting
requirements.

#### Prerequisites

Before submitting any vital signs Observation:
- A corresponding Patient resource **must exist** on the CIBMTR FHIR server.
- The Observation **must reference** the Patient using `Observation.subject.reference`.

Vital signs Observations submitted without a valid Patient reference will be rejected.

---

#### Vital Signs Observation Submission (CIBMTR Server Workflow)

Vital signs are submitted as FHIR Observation resources using the Direct FHIR Service API.
Each Observation represents a single measurement or a composite measurement (e.g., blood
pressure) taken at a specific point in time.

Key requirements include:
- `Observation.status` SHOULD be set to `final` for completed measurements.
- `Observation.code` SHALL use the appropriate LOINC code for the vital sign being reported.
- `Observation.effectiveDateTime` SHALL reflect when the measurement was taken.
- `Observation.value[x]` SHALL be populated for single-value vital signs.
- `Observation.component` SHALL be used for composite vital signs.
